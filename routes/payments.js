...
const { calcAmount } = require('../model/Payment');
...
router.get('/estimate-amount', identityManager(['user', 'manager', 'admin']), async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(req.query.parkingAreaId)) {
    return res.status(400).json({ apiId: req.apiId, statusCode: 400, message: 'Failure', error: PARK_AREA_CONSTANTS.NOT_FOUND });
  }
  ...
  const totalAmount = calcAmount(hours, days, ammountPerhour);
