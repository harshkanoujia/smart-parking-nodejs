...
router.get('/', async ( req , res)=>{                                                       //All users accounts
    try {
        ...
        if (allUsers.length === 0)   return res.status(400).send({ msg: 'No User Found !'})
...
router.post('/signup', async ( req , res)=>{                                                //User can signup 
    try {
        ...
        if (error) return res.status(400).send({msg: 'Validation failed', err: error.details[0].message})
        ...
        res.status(200).send({msg: 'User Created Successfully', User : savedUser})
    } catch (error) {
        console.log(error);
...
router.post('/login', async ( req , res)=>{                                                 //User can login
    try {
        ...
        if (!checkUser) return res.status(400).send({msg: "Email not found"})  
        ...
        if (!verifyPassword) return res.status(400).send({ err: 'Password not match !'})
        ...
        // res.status(200).send({msg: 'User Verify Successfully', Token : token})
...
router.post('/logout', auth, async (req, res) =>{                                 //User can logout with providing email and token in headers
    try {
        if (req.user.email !== req.body.email.trim().toLowerCase() ) {
            return res.status(400).send({ msg: 'Email in Token does not match with provided email'})
        }
        const checkUser = await User.findOne({ email: req.body.email.trim().toLowerCase() })                                 
        if (!checkUser) return res.status(400).send({ err: "Invalid email" })

        checkUser.token = ""
        await checkUser.save()
        
        res.status(200).send({ msg: "Successfully Logout", "User": checkUser })
...
router.put('/:id', async ( req , res)=>{                                                    //User can update there account
    ...
    try {
        ...
        if (!savedUser) return res.status(400).send({msg: "ID not found"})

        res.status(201).send({msg: 'User Updated Successfully', User : savedUser})
    } catch (error) {
        console.log(error);
        if (error.name === 'ValidationError') {
            return res.status(400).send({ msg: 'Validation failed', err: error.message });
...
router.delete('/:id', async ( req , res)=>{                                                 //User can delete there account
    ...
    try {
        ...
        if (!deleteUser) return res.status(400).send({msg: "ID not found"})

        res.status(200).send({msg: 'User Deleted Successfully', User : deleteUser})
...
router.get('/:id', async ( req , res)=>{                                                    //User found by its Id
    ...
    try {
        ...
        if (!checkUsers) return res.status(400).send({err: "ID not found"})
        
        res.status(200).send({'User': checkUsers })
    } catch (error) {
        console.log(error);
        res.status(500).send({msg : 'Server did not respond', err: error.message}) 
    }
